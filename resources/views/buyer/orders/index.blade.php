...
@php
    ...
    $paymentMap = [
        'cod' => ['COD', 'bg-gray-100 text-gray-600'],
        'qris' => ['QRIS', 'bg-emerald-100 text-emerald-700'],
    ];
    $paymentStatusMap = [
        'pending' => ['Belum Upload', 'bg-gray-100 text-gray-600'],
...
<div class="space-y-5 fade-up">
    ...
    <div class="space-y-2 lg:space-y-3">
        ...
        @php [$statusLabel, $statusClass] = $statusMap[$order->status] ?? [$order->status,'bg-gray-100 text-gray-600']; [$paymentLabel, $paymentClass] = $paymentMap[$order->payment_method ?? 'cod'] ?? ['COD', 'bg-gray-100 text-gray-600']; [$paymentStatusLabel, $paymentStatusClass] = $paymentStatusMap[$order->payment_status ?? 'pending'] ?? [strtoupper($order->payment_status ?? 'pending'), 'bg-gray-100 text-gray-600']; @endphp
        <div class="bg-white rounded-xl lg:rounded-2xl border border-gray-100 p-3 lg:p-4 sm:p-5 flex flex-col sm:flex-row gap-3 lg:gap-4 hover:shadow-md hover:border-green-200/50 transition-all">
            ...
            <div class="flex-1 min-w-0">
                <div class="flex flex-wrap items-center gap-1.5 lg:gap-2 mb-0.5 lg:mb-1">
                    ...
                    <span class="px-2 lg:px-2.5 py-0.5 rounded-md lg:rounded-lg text-[9px] lg:text-[10px] font-black uppercase {{ $paymentClass }}">{{ $paymentLabel }}</span>
                    <span class="px-2 lg:px-2.5 py-0.5 rounded-md lg:rounded-lg text-[9px] lg:text-[10px] font-black uppercase {{ $paymentStatusClass }}">{{ $paymentStatusLabel }}</span>
                </div>
