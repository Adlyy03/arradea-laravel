...
@section('title', 'Pembayaran QRIS — Arradea')
@section('page_title', 'Pembayaran QRIS')
...
<div class="max-w-2xl mx-auto space-y-4 fade-up">
    {{-- Back Button --}}
    <a href="{{ route('buyer.cart') }}" class="inline-flex items-center gap-2 text-sm font-bold text-gray-500 hover:text-gray-900 transition group">
    ...
    <div class="bg-red-50 border border-red-200 rounded-2xl p-6 text-center">
        ...
        <a href="{{ route('buyer.cart') }}" class="mt-4 inline-block px-6 py-3 rounded-xl font-bold text-sm text-white transition hover:opacity-90" style="background:#72bf77">
            Kembali ke Keranjang
        </a>
    </div>
    @else
    <div class="bg-white rounded-2xl lg:rounded-3xl border border-gray-100 overflow-hidden shadow-sm">
        ...
        <div class="p-6 lg:p-8 space-y-6">
            ...
            <div class="bg-gradient-to-br from-primary-50 to-green-50 rounded-2xl p-6 border border-primary-100">
                <img src="{{ asset('storage/'.$singleSeller->qris_image) }}" 
                     alt="QRIS {{ $singleSeller->payment_name }}" 
            ...
            <div class="bg-gray-50 rounded-2xl p-5 space-y-3">
                <div class="flex items-center justify-between pb-3 border-b border-gray-200">
                    ...
                    <span class="text-base font-black text-gray-900">{{ $singleSeller->payment_name }}</span>
                </div>
                @if($singleSeller->payment_type)
                <div class="flex items-center justify-between">
                    <span class="text-sm font-bold text-gray-500">Jenis Pembayaran</span>
                    <span class="text-sm font-bold text-gray-700">{{ strtoupper($singleSeller->payment_type) }}</span>
                </div>
                @endif
                @if($singleSeller->payment_number)
                <div class="flex items-center justify-between">
                    <span class="text-sm font-bold text-gray-500">Nomor Akun</span>
                    <span class="text-sm font-bold text-gray-700">{{ $singleSeller->payment_number }}</span>
                </div>
                @endif
                <div class="flex items-center justify-between pt-3 border-t border-gray-200">
                    <span class="text-sm font-bold text-gray-500">Total Pembayaran</span>
                    <span class="text-2xl font-black" style="color:#72bf77">Rp {{ number_format($totalFinal ?? 0,0,',','.') }}</span>
                </div>
            </div>
                ...
                <div class="flex items-start gap-3">
                    ...
                    <div class="flex-1">
                        ...
                        <ol class="text-sm text-blue-700 space-y-2 list-decimal list-inside">
                            ...
                            <li>Pastikan nominal pembayaran <strong>Rp {{ number_format($totalFinal ?? 0,0,',','.') }}</strong></li>
            ...
            {{-- Upload Form --}}
            <form action="{{ route('buyer.cart.checkout') }}" method="POST" id="qris-checkout-form" class="space-y-4">
                @csrf
