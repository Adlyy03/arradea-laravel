...
<div class="space-y-4 fade-up mobile-compact">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="greeting-title text-2xl lg:text-3xl font-black text-gray-900">Halo, {{ explode(' ', auth()->user()->name)[0] }}! <span class="inline-block animate-bounce">👋</span></h1>
            <p class="text-xs lg:text-sm text-gray-500 mt-1 font-medium">{{ now()->locale('id')->isoFormat('dddd, D MMMM Y') }}</p>
        </div>
        <a href="{{ route('buyer.products') }}" class="hidden sm:flex items-center gap-2 px-4 lg:px-6 py-2 lg:py-3 rounded-xl lg:rounded-2xl text-xs lg:text-sm font-bold text-white transition-all duration-300 hover:-translate-y-1 hover:shadow-xl active:scale-95" style="background:linear-gradient(135deg,#72bf77,#4db85a);box-shadow:0 8px 24px rgba(114,191,119,.3)">
    ...
    <div class="stat-card rounded-2xl lg:rounded-3xl overflow-hidden shadow-lg">
        <div class="flex items-center justify-between px-4 lg:px-6 py-3 lg:py-5 border-b border-gray-100/60">
            ...
            <a href="{{ route('buyer.orders') }}" class="text-[10px] lg:text-xs font-bold inline-flex items-center gap-1 group transition" style="color:#72bf77">
                Lihat Semua
        ...
        <div class="order-row flex items-center justify-between px-4 lg:px-6 py-3 lg:py-5 border-b border-gray-50/60">
            <div class="flex-1 min-w-0">
                <p class="order-text text-xs lg:text-sm font-bold text-gray-900 truncate">{{ $order->product->name ?? 'Pesanan #'.$order->id }}</p>
                <p class="order-meta text-[10px] lg:text-xs text-gray-400 mt-1">{{ $order->store->name ?? '-' }} · {{ $order->created_at->diffForHumans() }}</p>
            </div>
            <div class="flex items-center gap-2 lg:gap-4 ml-3 lg:ml-4 flex-shrink-0">
                <div class="text-right hidden sm:block">
                    ...
                    <p class="text-xs lg:text-sm font-black text-gray-900">Rp {{ number_format($order->total_price,0,',','.') }}</p>
                </div>
                <span class="px-2 lg:px-3 py-1 lg:py-1.5 rounded-lg lg:rounded-xl text-[9px] lg:text-xs font-black uppercase tracking-wider {{ $statusClass }}">{{ $statusLabel }}</span>
                <a href="{{ route('chat.show', $order) }}" class="w-7 lg:w-9 h-7 lg:h-9 rounded-lg lg:rounded-xl flex items-center justify-center text-white text-xs transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg" style="background:linear-gradient(135deg,#72bf77,#4db85a)">
        ...
        <div class="flex flex-col items-center justify-center py-12 lg:py-20 text-center">
            ...
            <a href="{{ route('buyer.products') }}" class="px-4 lg:px-6 py-2 lg:py-3 rounded-xl lg:rounded-2xl text-xs lg:text-sm font-bold text-white transition-all duration-300 hover:-translate-y-1 hover:shadow-xl" style="background:linear-gradient(135deg,#72bf77,#4db85a)">Mulai Belanja</a>
        </div>
        @endforelse
    </div>
</div>
