<div class="space-y-3">
    ...
    <div class="bg-white rounded-xl border border-red-100 overflow-hidden hover:shadow-sm transition">
        <div class="p-4">
            <div class="flex items-start gap-3 mb-3">
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex items-start justify-between gap-2 mb-1">
                        ...
                        <span class="px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-wider bg-red-100 text-red-700 flex-shrink-0">
                            ❌ Ditolak
                        </span>
                    </div>
                    ...
                    <div class="flex items-center gap-2 text-[10px] text-gray-400">
                        ...
                        <span>Ditolak {{ $order->updated_at->diffForHumans() }}</span>
                    </div>
                </div>
                ...
                <a href="{{ asset('storage/'.$order->payment_proof) }}" target="_blank" class="flex-shrink-0">
                    <img src="{{ asset('storage/'.$order->payment_proof) }}" 
                         alt="Bukti" 
                         class="w-12 h-12 lg:w-14 lg:h-14 rounded-lg object-cover border border-gray-200 hover:border-red-500 transition">
                </a>
                @endif
            </div>
            ...
            <div class="rounded-lg bg-red-50 border border-red-200 p-3">
                <p class="text-[10px] font-bold uppercase tracking-wider text-red-900 mb-1">💬 Alasan Penolakan</p>
                <p class="text-xs text-red-700 leading-relaxed">{{ $order->rejected_reason }}</p>
            </div>
            @endif
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-dashed border-gray-200 p-10 text-center">
        ...
        <p class="text-base font-black text-gray-900">Belum ada pembayaran ditolak</p>
        <p class="text-sm text-gray-500 mt-1">Riwayat pembayaran yang ditolak akan muncul di sini.</p>
    </div>
    @endforelse
</div>
