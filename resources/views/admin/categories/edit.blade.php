...
<div class="p-4 sm:p-6 lg:p-8 space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Perbarui informasi kategori {{ $category->name }}</p>
        </div>
    </div>
        ...
        <form action="{{ route('admin.categories.update', $category) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <div id="fileTab" class="space-y-3">
                    ...
                    {{-- Show existing image if it's a URL (not emoji) --}}
                    @if($category->image && (strpos($category->image, '/') !== false || strpos($category->image, 'http') !== false))
                    <div id="existingImagePreview">
                        <p class="text-xs text-gray-500 mb-2">Gambar saat ini:</p>
                        <img src="{{ $category->image }}" alt="{{ $category->name }}" class="w-32 h-32 object-cover rounded-lg">
                    </div>
                    @endif
                ...
                <div id="textTab" class="hidden">
                    <input type="text" 
                           id="image_text" 
                           name="image_text" 
                           value="{{ old('image_text', (strpos($category->image, '/') === false && strpos($category->image, 'http') === false) ? $category->image : '') }}"
            ...
            <div class="bg-blue-50 border border-blue-200 rounded-xl p-4 flex items-start gap-3">
                ...
                <div class="text-sm text-blue-800">
                    <p class="font-bold">Kategori ini memiliki {{ $category->products()->count() }} produk</p>
