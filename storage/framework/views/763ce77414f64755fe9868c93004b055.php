<?php $__env->startSection('title', 'Pesanan Saya - Arradea'); ?>
<?php $__env->startSection('page_title', 'Konfirmasi Pesanan Pelanggan'); ?>
...
<div class="space-y-8">
    ...
    <div class="bg-white rounded-3xl p-8 shadow-sm border border-gray-100 space-y-8">
        <div class="flex justify-between items-center px-4">
            ...
            <div class="text-gray-400 font-bold uppercase tracking-widest text-[10px]">Total: <?php echo e(Auth::user()->orders()->count()); ?></div>
        </div>
        ...
        <div class="overflow-x-auto shadow-inner rounded-2xl border border-gray-50">
            <table class="w-full text-left font-['Plus_Jakarta_Sans']">
                ...
                <tbody class="divide-y divide-gray-50">
                    <?php $__empty_1 = true; $__currentLoopData = Auth::user()->orders()->with('store')->latest()->get(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $order): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-primary-50/20 transition duration-300">
                            <td class="px-6 py-5">
                                <div class="flex items-center gap-4">
                                    ...
                                    <div class="space-y-1">
                                        <p class="font-black text-base text-gray-900 leading-tight">ARRD-<?php echo e(str_pad($order->id, 6, '0', STR_PAD_LEFT)); ?></p>
                                        <p class="text-sm font-medium text-gray-400">Toko: <?php echo e($order->store->name ?? 'Arradea Central'); ?></p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-5">
                                <p class="text-lg font-black text-gray-900 tracking-tight">Rp <?php echo e(number_format($order->total_price, 0, ',', '.')); ?></p>
                            </td>
                            <td class="px-6 py-5">
                                <?php
                                    $orderStatusColors = [
                                ...
                                ?>
                                <span class="<?php echo e($orderStatusColors[$order->status] ?? 'bg-gray-100 text-gray-500'); ?> px-5 py-2.5 rounded-3xl text-[10px] font-black uppercase tracking-widest"><?php echo e($orderStatusLabels[$order->status] ?? $order->status); ?></span>
                            </td>
                            <td class="px-6 py-5">
                                <div class="flex gap-3">
                                    <a href="<?php echo e(route('chat.show', $order)); ?>" class="px-4 py-2 bg-blue-600 text-white rounded-xl text-xs font-bold uppercase tracking-wider hover:scale-105 transition-all">Chat Seller</a>
                                    <a href="<?php echo e(route('buyer.orders.show', $order)); ?>" class="px-4 py-2 bg-gray-100 text-gray-600 rounded-xl text-xs font-bold uppercase tracking-wider hover:bg-gray-200 transition-all">Detail</a>
                                    <?php if($order->status === 'pending'): ?>
                                        <form action="<?php echo e(route('buyer.orders.cancel', $order)); ?>" method="POST" onsubmit="return confirmSubmit(event, <?php echo \Illuminate\Support\Js::from('Yakin ingin membatalkan pesanan ini?')->toHtml() ?>);">
                                            <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.dashboard', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\arradea-laravel\resources\views/buyer/orders.blade.php ENDPATH**/ ?>