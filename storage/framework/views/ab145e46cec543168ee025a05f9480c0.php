<?php $__env->startSection('title', 'Detail Produk - Arradea'); ?>
<?php $__env->startSection('page_title', 'Detail & Pesan Produk'); ?>
...
<div class="space-y-4 lg:space-y-6">
    ...
    <div class="flex items-center gap-2 lg:gap-3">
        <a href="<?php echo e(route('buyer.products')); ?>" class="product-detail-back inline-flex items-center gap-1.5 lg:gap-2 px-3 lg:px-4 py-2 lg:py-2.5 bg-white hover:bg-gray-50 border border-gray-200 rounded-lg lg:rounded-xl font-bold text-xs lg:text-sm text-gray-700 transition-all hover:shadow-md active:scale-95 group">
        ...
        <nav class="product-detail-breadcrumb text-xs lg:text-sm text-gray-500">
            <a href="<?php echo e(route('buyer.products')); ?>" class="hover:text-primary-600 transition">Produk</a>
            <span class="mx-1 lg:mx-2">/</span>
            <span class="text-gray-900 font-semibold"><?php echo e(Str::limit($product->name, 20)); ?></span>
        </nav>
    </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6 lg:gap-10">
            ...
            <div class="relative">
                <div class="product-detail-image aspect-square rounded-xl lg:rounded-2xl overflow-hidden bg-gray-50 border border-gray-100 shadow-lg">
                    <img src="<?php echo e($product->image ?? 'https://via.placeholder.com/700?text=No+Image'); ?>" alt="<?php echo e($product->name); ?>" class="w-full h-full object-cover object-center block">
                </div>
                <?php $basePricing = $product->calculatePricing('default', 1); ?>
                <?php if(($basePricing['discount_percent'] ?? 0) > 0): ?>
                    <div class="product-detail-discount-badge absolute top-3 lg:top-4 right-3 lg:right-4 bg-red-500 text-white px-2 lg:px-3 py-1 lg:py-1.5 rounded-full text-[10px] lg:text-xs font-black shadow-lg">
                        -<?php echo e($basePricing['discount_percent']); ?>%
                    </div>
                <?php endif; ?>
            </div>
            ...
            <div class="space-y-4 lg:space-y-6">
                ...
                <div class="space-y-1.5 lg:space-y-2">
                    <div class="product-detail-store flex items-center gap-1.5 lg:gap-2">
                        ...
                        <span class="text-[10px] lg:text-xs font-bold uppercase tracking-wider text-gray-500"><?php echo e($product->store->name ?? 'Arradea'); ?></span>
                        <?php if($product->store): ?>
                            <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] lg:text-[10px] font-black <?php echo e($product->store->store_status === 'open' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'); ?>">
                                <span class="w-1 lg:w-1.5 h-1 lg:h-1.5 rounded-full <?php echo e($product->store->store_status === 'open' ? 'bg-green-500' : 'bg-gray-400'); ?> animate-pulse"></span>
                                <?php echo e($product->store->store_status === 'open' ? 'Buka' : 'Tutup'); ?>

                            </span>
                        <?php endif; ?>
                    </div>
                    <h1 class="product-detail-title text-2xl lg:text-3xl lg:text-4xl font-black text-gray-900 leading-tight"><?php echo e($product->name); ?></h1>
                </div>
                ...
                <div class="product-detail-price-box bg-gradient-to-br from-primary-50 to-green-50 border border-primary-100 rounded-xl lg:rounded-2xl p-4 lg:p-5">
                    ...
                    <?php if(($basePricing['discount_percent'] ?? 0) > 0): ?>
                        <div class="flex items-baseline gap-2 lg:gap-3">
                            <span class="product-detail-price text-2xl lg:text-3xl lg:text-4xl font-black text-primary-700">Rp <?php echo e(number_format($basePricing['unit_final'], 0, ',', '.')); ?></span>
                            <span class="product-detail-price-old text-sm lg:text-lg font-bold text-gray-400 line-through">Rp <?php echo e(number_format($basePricing['unit_original'], 0, ',', '.')); ?></span>
                        </div>
                        <p class="text-[10px] lg:text-xs font-bold text-primary-600 mt-1">Hemat Rp <?php echo e(number_format($basePricing['unit_original'] - $basePricing['unit_final'], 0, ',', '.')); ?></p>
                    <?php else: ?>
                        <span class="product-detail-price text-2xl lg:text-3xl lg:text-4xl font-black text-primary-700">Rp <?php echo e(number_format($product->price, 0, ',', '.')); ?></span>
                    <?php endif; ?>
                    <div class="product-detail-stock flex items-center gap-1.5 lg:gap-2 mt-2 lg:mt-3 pt-2 lg:pt-3 border-t border-primary-100">
                        ...
                        <span class="text-xs lg:text-sm font-bold text-gray-600">Stok: <span class="text-gray-900"><?php echo e($product->stock); ?> unit</span></span>
                    </div>
                </div>
                ...
                <div class="product-detail-desc bg-gray-50 rounded-lg lg:rounded-xl p-4 lg:p-5 border border-gray-100">
                    ...
                    <p class="text-gray-700 leading-relaxed text-xs lg:text-sm"><?php echo e($product->description ?? 'Tidak ada deskripsi.'); ?></p>
                </div>

                <!-- Purchase Form / Actions -->
                <?php if(auth()->guard()->check()): ?>
                    <?php if($product->store && (int) auth()->id() === (int) $product->store->user_id): ?>
                    ...
                    <?php elseif($product->store && $product->store->store_status !== 'open'): ?>
                        <div class="bg-red-50 border-2 border-red-200 rounded-2xl p-6 flex items-start gap-4">
                            ...
                            <div>
                                ...
                                <?php if($product->store->open_time && $product->store->close_time): ?>
                                    <p class="text-red-600 text-xs mt-2 font-semibold">Jam operasional: <?php echo e(substr($product->store->open_time, 0, 5)); ?> - <?php echo e(substr($product->store->close_time, 0, 5)); ?></p>
                                <?php endif; ?>
                            </div>
                        </div>
                    <?php else: ?>
                        <form action="<?php echo e(route('buyer.cart.store')); ?>" method="POST" class="space-y-5 bg-white border-2 border-gray-200 rounded-2xl p-6">
                            <?php echo csrf_field(); ?>
                                ...
                                <div class="space-y-2">
                                    ...
                                    <select name="variant_key" class="w-full border-2 border-gray-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 font-semibold text-gray-700 transition">
                                        ...
                                        <?php $__currentLoopData = $product->variants; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $variant): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            ...
                                            ?>
                                            <option value="<?php echo e($variant['key'] ?? ''); ?>">
                                                <?php echo e($variant['name'] ?? 'Varian'); ?> - Rp <?php echo e(number_format($variantPricing['unit_final'], 0, ',', '.')); ?>
                            ...
                            <div class="space-y-2">
                                ...
                                <input type="number" name="quantity" value="1" min="1" max="<?php echo e($product->stock); ?>" class="w-32 border-2 border-gray-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 font-bold text-gray-900 transition" required>
                            </div>
                            ...
                            <div class="flex flex-col sm:flex-row gap-3 pt-2">
                                <button type="submit" class="flex-1 px-6 py-4 bg-gradient-to-r from-primary-600 to-green-600 text-white rounded-xl font-black text-base hover:from-primary-700 hover:to-green-700 shadow-lg shadow-primary-200 transition-all active:scale-95 flex items-center justify-center gap-2 <?php echo e($product->stock === 0 ? 'opacity-50 cursor-not-allowed' : ''); ?>" <?php echo e($product->stock === 0 ? 'disabled' : ''); ?>>
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z"/>
                                    </svg>
                                    <?php echo e($product->stock === 0 ? 'Stok Habis' : 'Tambah ke Keranjang'); ?>

                                </button>
                                <a href="<?php echo e(route('buyer.cart')); ?>" class="px-6 py-4 bg-white text-gray-700 border-2 border-gray-200 rounded-xl font-black text-base hover:bg-gray-50 hover:border-gray-300 transition-all text-center flex items-center justify-center gap-2">
                                    Lihat Keranjang
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <?php else: ?>
                    <div class="bg-gradient-to-br from-blue-50 to-indigo-50 border-2 border-blue-200 rounded-2xl p-6">
                        ...
                        <div class="flex gap-3">
                            <a href="<?php echo e(route('login')); ?>" class="flex-1 px-6 py-3 bg-primary-600 text-white rounded-xl font-black text-sm hover:bg-primary-700 transition-all text-center active:scale-95">Login</a>
                            <a href="<?php echo e(route('register')); ?>" class="flex-1 px-6 py-3 bg-white text-gray-700 border-2 border-gray-200 rounded-xl font-black text-sm hover:bg-gray-50 transition-all text-center active:scale-95">Daftar</a>
                        </div>
                    </div>
                <?php endif; ?>
            </div>
...
<?php echo $__env->make('layouts.dashboard', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\arradea-laravel\resources\views/buyer/products/show.blade.php ENDPATH**/ ?>