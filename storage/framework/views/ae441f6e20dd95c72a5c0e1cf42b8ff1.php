<?php $__env->startSection('title', 'Chat - Arradea'); ?>
...
<div class="max-w-2xl mx-auto space-y-4 fade-up">
    ...
    <a href="<?php echo e($backRoute); ?>" class="inline-flex items-center gap-2 text-sm font-bold text-gray-500 hover:text-gray-900 transition group">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        ...
        <div class="flex items-center justify-between p-3 lg:p-4 border-b border-gray-50" style="background:linear-gradient(to right,#f0faf1,#fff)">
            <div class="flex items-center gap-2 lg:gap-3 min-w-0 flex-1">
                <div class="w-9 lg:w-10 h-9 lg:h-10 rounded-xl lg:rounded-2xl flex items-center justify-center text-xs lg:text-sm font-black flex-shrink-0"
                    ...
                    <?php echo e(strtoupper(substr($withName, 0, 1))); ?>

                </div>
                <div class="min-w-0 flex-1">
                    <p class="font-black text-gray-900 text-xs lg:text-sm truncate"><?php echo e($withName); ?></p>
                    <p class="text-[9px] lg:text-[10px] font-bold text-gray-400 uppercase tracking-widest truncate">
                        Order #<?php echo e(str_pad($order->id, 6, '0', STR_PAD_LEFT)); ?> · <?php echo e($order->product->name ?? ''); ?>

                    </p>
                </div>
            </div>
            <div class="flex items-center gap-1.5 lg:gap-2 flex-shrink-0">
                <span class="<?php echo e($statusClass); ?> px-2 lg:px-2.5 py-0.5 lg:py-1 rounded-lg text-[9px] lg:text-[10px] font-black uppercase tracking-widest"><?php echo e($statusLabel); ?></span>
                <?php if($isSellerView && $order->status === 'pending'): ?>
                <form action="/web/order/<?php echo e($order->id); ?>/status" method="POST" class="inline">
        ...
        <div class="flex items-center gap-3 lg:gap-4 px-3 lg:px-4 py-2.5 lg:py-3 bg-gray-50/50 border-b border-gray-50">
            ...
            <img src="<?php echo e($order->product->image); ?>" alt="" class="w-9 lg:w-10 h-9 lg:h-10 rounded-lg lg:rounded-xl object-cover border border-gray-100 flex-shrink-0">
            <?php endif; ?>
            <div class="flex-1 min-w-0">
                <p class="text-[11px] lg:text-xs font-black text-gray-700 truncate"><?php echo e($order->product->name ?? '—'); ?></p>
                <p class="text-[9px] lg:text-[10px] text-gray-400"><?php echo e($order->quantity); ?>× · Rp <?php echo e(number_format($order->total_price, 0, ',', '.')); ?></p>
            </div>
            <span class="text-[9px] lg:text-[10px] font-bold text-gray-400 flex-shrink-0"><?php echo e($order->created_at->format('d M Y')); ?></span>
        </div>
        ...
        <div id="messages"
            ...
            ?>
            <div class="flex <?php echo e($isOwn ? 'justify-end' : 'justify-start'); ?>" data-message-id="<?php echo e($message->id); ?>">
                <?php if(!$isOwn): ?>
                ...
                <?php endif; ?>
                <div class="max-w-[75%] <?php echo e($isOwn ? 'items-end' : 'items-start'); ?> flex flex-col gap-0.5">
                    <div class="px-4 py-2.5 rounded-2xl text-sm font-medium leading-relaxed
                        <?php echo e($isOwn
                         ...
                         style="<?php echo e($isOwn ? 'background:#3fa348' : ''); ?>">
                        <?php echo nl2br(e(trim($cleanMessage))); ?>

                        
                        <?php if($paymentProofPath): ?>
                        <div class="mt-2 rounded-xl overflow-hidden border-2 <?php echo e($isOwn ? 'border-white/20' : 'border-gray-200'); ?>">
                            <a href="<?php echo e(asset('storage/'.$paymentProofPath)); ?>" target="_blank" class="block">
                                <img src="<?php echo e(asset('storage/'.$paymentProofPath)); ?>" alt="Bukti Pembayaran" class="w-full max-w-xs rounded-lg">
                            </a>
                            <div class="px-2 py-1.5 text-[10px] font-bold <?php echo e($isOwn ? 'bg-white/10 text-white' : 'bg-gray-50 text-gray-600'); ?>">
                                📸 Bukti Pembayaran - Klik untuk memperbesar
                            </div>
                        </div>
                        <?php endif; ?>
                    </div>
                    <span class="text-[9px] font-bold text-gray-400 px-1"><?php echo e($message->created_at->diffForHumans()); ?></span>
                </div>
            </div>
            ...
            <div id="empty-chat-state" class="flex flex-col items-center justify-center h-full text-center py-8">
                ...
                <p class="text-xs text-gray-400 mt-1">Mulai percakapan dengan <?php echo e($withName); ?></p>
            </div>
            <?php endif; ?>
        ...
        <div class="p-3 lg:p-4 border-t border-gray-100" style="background:#fafafa">
            <form id="chat-form" action="<?php echo e(route('chat.store', $chat)); ?>" method="POST">
                <?php echo csrf_field(); ?>
...
<script>
(function () {
    const chatId = Number(<?php echo json_encode($chat->id, 15, 512) ?>);
    const currentUserId = Number(<?php echo json_encode(auth()->id(), 15, 512) ?>);
    const csrfToken = <?php echo json_encode(csrf_token(), 15, 512) ?>;
    const withName = <?php echo json_encode($withName, 15, 512) ?>;
...
<?php echo $__env->make('layouts.dashboard', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\arradea-laravel\resources\views/chat/show.blade.php ENDPATH**/ ?>